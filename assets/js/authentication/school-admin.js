//TABLE REFERENCE
var schoolDatabaseReference = db.ref("schoolAdmins");
var schoolDatabaseReference1 = db.ref("schools");

var arr = [];

schoolDatabaseReference1.on("child_added", function (schData) {
  // console.log(schData.val().sch_id);
  // arr = schData.val().sch_id;
  // console.log(arr);
  const propertyValues = Object.values(schData.val());
...
function addNewadmin() {
  ...
  var dg = document.getElementById("designation");
  var adminDg = dg.options[dg.selectedIndex].text;
...
>
  ...
  >
    ...
    <!--[if (IE)]></div><![endif]-->
      ...
      alert("Welcome Email has been Sent to the Registered School Admin")
    );
  });
...
$(document).on("click", ".editTch", function () {
  ...
  schoolDatabaseReference.child(tch_id).once("value", function (sch) {
    ...
    modal_body += "</div>";
    modal_body += '<div class="form-group">';
    modal_body += "<label>admin Designation</label>";
    modal_body +=
      '<select class="form-control" id="edit-admin-designation" value="' +
      sch.val().adminDg +
      '">';
    modal_body += "<option>Principal</option>";
    modal_body += "<option>Assistant Professor</option>";
    modal_body += "<option>Professor</option>";
    modal_body += "</select>";
    modal_body += "</div>";
