...
public class UserInterface {
	...
	public UserInterface(Pane theRoot) {
		 ...
		 */
		setupLabelUI(label_loginHead, "Calibri", 44, Main.WINDOW_WIDTH, Pos.BASELINE_CENTER, 0, verticalGap);
		...
		submitLogin.setOnAction(ev -> {
			try {
				if (checkInput()) {
					Firebase.connectDB(text_userID.getText().toString(), text_pass.getText().toString());
					try {
						Thread.sleep(10000);
					} catch (InterruptedException e) {
					}
					System.out.println("Condition: " + Firebase.loginValid);
					if (Firebase.loginValid) {
						System.out.println("Connecting Drive Now");
		 ...
		 * Dashboard Elements
		 */
		setupLabelUI(label_headTD, "Calibri", 44, Main.WINDOW_WIDTH, Pos.CENTER, 0, verticalGap);
